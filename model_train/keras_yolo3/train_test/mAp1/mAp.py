...
class MAp():
    def __init__(self,classnames,xml_path,jpg_path,predictFile):
        self.labels = classnames
        self.xmlTestPath = xml_path
        self.jpgTestPath = jpg_path
        self.wfile = predictFile


    def sumXMlObjects(self):
        labels_dict = {}
        for label in self.labels:
            labels_dict[label] = 0
        files = os.listdir(self.xmlTestPath)
        for file in files:
            f = self.xmlTestPath + file
    ...
    def xmlFilesDict(self,xmlTestPath):
        file_dict = {}
    ...
    def xmlObjects(self,file,object_dict):
        f = file
        tree = ET.parse(f)  # 打开xml文档
        root = tree.getroot()  # 获得root节点
        filename = root.find('filename').text
    ...
    def xmlObjectsLocations(self,f):
    ...
    def IOU (self,predictBox,Box,overthroud=0.45):
        cx1 = predictBox[0]
        cy1 = predictBox[1]
        cx2 = predictBox[2]
        cy2 = predictBox[3]
    ...
    def classPrecesion(self,predictDict,xmlDict,object_Dict):
        classPrecesion_dict = {}
        for predict in object_Dict:
            ...
            for xmlBox in xmlBoxs:
                for preBox in preBoxs:
                    bFalg = self.IOU(list(preBox), list(xmlBox))
                    if bFalg:
    ...
    def averagePrecision(self):
        ...
        for infoP in listPre:
            ...
            xmlFileName = self.xmlTestPath+str(os.path.splitext(fileName)[0])+".xml"
            xmlFileDict = self.xmlObjectsLocations(xmlFileName)
            classP_dict = {}
            for label in self.labels:
                classP_dict[label] = 0
            classPrecesion_dict = self.classPrecesion(predictDict,xmlFileDict,classP_dict)
            # print (str(fileName))
        ...
        for label in self.labels:
            fileC_dict[label] = 0
        file_dict = self.xmlFilesDict(self.xmlTestPath)
        for l in fileC_dict:
    ...
    def meanAveragePrecesion(self,classAveragePrecesion_dict):
    ...
    def cal_Ap_mAp(self,classnames,xml_path,jpg_path,predictFile):
        classAveragePrecesion_dict = self.averagePrecision()
        mAp = self.meanAveragePrecesion(classAveragePrecesion_dict)
        return classAveragePrecesion_dict,mAp
