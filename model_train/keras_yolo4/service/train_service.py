...
import shutil
from model_train.keras_yolo4.train_test import train
from model_train.keras_yolo4.train_test.mAp1 import predict,mAp
import demjson
import traceback
...
def check_path(shop_id,batch_id,online_batch_id,type):
    if online_batch_id is not None and type == 1:
        online_model = os.path.join(str(config.yolov4_train_params['model_dir']).format(shop_id,online_batch_id),str(shop_id)+"_"+str(online_batch_id)+".h5")
    ...
    model_dir = str(config.yolov4_train_params['model_dir']).format(shop_id,batch_id)
    log_dir = str(config.yolov4_train_params['log_dir']).format(shop_id, batch_id)
    convert_path = str(config.yolov4_train_params['convert_path']).format(shop_id, batch_id)
    wfile_path = str(config.yolov4_train_params['predict_wfile_path']).format(shop_id, batch_id)
...
def cal_mAp(shop_id,batch_id,classnames):
    default_config_params = config.yolov4_train_params['default_config_params']
    good_select_config_params_mAP = 0
    good_select_config_params = ''
    for config_params in default_config_params:
        wfile = (config.yolov4_train_params['predict_wfile']).format(shop_id,batch_id,config_params)
        testJpgPath = str(config.yolov4_train_params['JPEGImages_path']).format(shop_id,batch_id)
        testXmlPath = str(config.yolov4_train_params['Annotations_path']).format(shop_id,batch_id)
        ...
        model_path = os.path.join(str(config.yolov4_train_params['model_dir']).format(shop_id,batch_id),str("{}_{}.h5").format(shop_id,batch_id))
...
def save_train_table(group_id, model_id, type,train_los_time=0,val_los_time=0,good_config_params='',all_config_params='',status=1,des_msg=''):
    ...
    model_path = os.path.join(str(config.yolov4_train_params['model_dir']).format(group_id,model_id),str("{}_{}.h5").format(group_id,model_id))
